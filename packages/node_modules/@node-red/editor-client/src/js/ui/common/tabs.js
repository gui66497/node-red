...
RED.tabs = (function() {
    ...
    function createTabs(options) {
        ...
        if (options.addButton) {
            // 适配版本 不显示add flow按钮
            if (window.location.hash.indexOf("only") < 0) {
                ...
                // 适配 隐藏header操作栏
                $("#red-ui-main-container").css("top", "0px");
            }
        ...
        return {
            addTab: function(tab,targetIndex) {
                ...
                // 参数带only说明是适配版本 隐藏其它tab
                var currentHash = window.location.hash;
                if (currentHash.indexOf("only") > -1) {
                    var pathId = currentHash.substring(6, currentHash.indexOf("only") - 1);
                    if (tab.id !== pathId) {
                        li.addClass("hide");
                    }
                }
