...
RED.editor = (function() {
    ...
    /**
     * ls-filter-grok 增加一行"解析规则" 调用zzjg接口获取规则列表
     */
    function grokMatchExtend() {
        console.log('$("#node-input-ls_match")', $("#node-input-ls_match").val());
        var originMatchStr = $("#node-input-ls_match").val();
        var selectIdArr = [];
        if (originMatchStr.indexOf("[") > -1 && originMatchStr.indexOf("]") > -1) {
            // 数组
            var arrStr = originMatchStr.substring(originMatchStr.indexOf("[") + 1, originMatchStr.lastIndexOf("]") - 1).trim();
            selectIdArr = arrStr.split(",");

        } else if (originMatchStr.indexOf("message") > -1) {
            // 单个
            var temp1 = originMatchStr.substring(0, originMatchStr.lastIndexOf("}"));
            var str = temp1.split("=>")[1].trim().replace(/^\"|\"$/g, '');
            selectIdArr.push(str);
        }
        console.log('selectIdArr', selectIdArr);

        var jsonParam = {"entity": {"pageNo": 1, "pageSize": 100}};
        var rules = [];
        $.ajax({
            method: 'POST',
            contentType: 'application/json',
            url: 'https://192.168.1.36:8443/zzjg/logstash/getMatchRulePage',
            data: JSON.stringify(jsonParam),
            success: function (data) {
                console.log('getMatchRulePage:', data);
                rules = data.obj.list;
                var arr = [];
                for (var i = 0; i < rules.length; i++) {
                    arr.push({id:rules[i].content, text: rules[i].ruleName})
                }
                $("#grok-match-select").select2({
                    data: arr,
                    placeholder: '请选择解析规则'
                });

                if (selectIdArr.length > 0) {
                    $("#grok-match-select").val(selectIdArr).trigger("change");
                }
                // 绑定change事件
                $('#grok-match-select').on('change', function (e) {
                    console.log('change', e);
                    // 获取当前所选 构造match语句
                    var selected = $('#grok-match-select').select2('data');
                    var matchArr = [];
                    for (var j = 0; j < selected.length; j++) {
                        matchArr.push(selected[j].id);
                    }
                    var matchStr = '';
                    if (matchArr.length > 0) {
                        if (matchArr.length === 1) {
                            matchStr = '{ "message" => "' + matchArr.join(",") + '" }';
                        } else {
                            matchStr = '{ "message" => [ ' + matchArr.join(",") + ' ] }';
                        }
                    }
                    $("#node-input-ls_match").val(matchStr);
                });

            },
            error: function(jqXHR,textStatus,errorThrown) {
                console.log("Unexpected error saving user settings:",jqXHR.status,textStatus);
                alert("后台引擎连接失败：" + jqXHR.status + " " + textStatus);
            }
        });
    ...
    function showEditDialog(node) {
        console.log('showEditDialog', node);
