...
var runtime;
var nr2ls = require("@node-red/util/lib/nr2ls");
...
var api = module.exports = {
    ...
    */
     ...
     * @returns {Promise}
     */
    generateConfig: function(opts) {
        return new Promise(function (resolve,reject) {
            var flow = runtime.nodes.getFlow(opts.id);
            console.log('generateConfig', flow);
            if (flow) {
                // 调用nr2ls生成logstash配置文件
                let config = nr2ls.generate(flow);
                console.log('generateConfig result:', config);
                runtime.log.audit({event: "flow.generate",id:opts.id}, opts.req);
                return resolve(config);
            } else {
                runtime.log.audit({event: "flow.generate",id:opts.id,error:"not_found"}, opts.req);
                var err = new Error();
                err.code = "not_found";
                err.status = 404;
                return reject(err);
    ...
    getMatchRules: function() {
        return new Promise(function (resolve, reject) {
            let options = {
                url: "http://backend:8084/zzjg/logstash/getMatchRulePage",
                method: 'POST',
                json: true,
                headers: {
                    "content-type": "application/json",
                },
                body: {"entity": {"pageNo": 1, "pageSize": 500}}
            };
